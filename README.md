...
- Mastodon API provide a route to stream timelines, this is an opportunity to create a websocket or server-sent events with the Mastodon API that would allow to have real-time notifications each time a new post is published on the Fediverse with an `#elixir` hashtag ; this would display a call-to-action to the user in order to refresh the feed.
